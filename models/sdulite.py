"""
U-Lite-DropBlock: U-Lite with Structured Dropout (DropBlock)
Combines the lightweight axial depthwise convolutions of U-Lite 
with the improved regularization of DropBlock2D from SD-UNet
"""
...
class BottleNeckBlockWithDropBlock(nn.Module):
    """U-Lite Bottleneck Block enhanced with DropBlock"""
    
    def __init__(self, dim, block_size=7, keep_prob=0.9, activation='gelu'):
        super().__init__()
...
class ConfigurableULiteDropBlock(nn.Module):
    """
    Configurable U-Lite with DropBlock for hyperparameter optimization
    Combines U-Lite's efficient architecture with SD-UNet's structured dropout
    """
    ...
    def __init__(self, 
        ...
        self.e1 = EncoderBlockWithDropBlock(
            base_channels, base_channels * 2,
            block_size=block_size, keep_prob=keep_prob, activation=activation
        )
        self.e2 = EncoderBlockWithDropBlock(
            base_channels * 2, base_channels * 4,
            block_size=block_size, keep_prob=keep_prob, activation=activation
        )
        self.e3 = EncoderBlockWithDropBlock(
            base_channels * 4, base_channels * 8,
            block_size=block_size, keep_prob=keep_prob, activation=activation
        )
        self.e4 = EncoderBlockWithDropBlock(
            base_channels * 8, base_channels * 16,
            block_size=block_size, keep_prob=keep_prob, activation=activation
        )
        self.e5 = EncoderBlockWithDropBlock(
            base_channels * 16, base_channels * 32,
            block_size=block_size, keep_prob=keep_prob, activation=activation
        )

        # Bottleneck with DropBlock
        self.b5 = BottleNeckBlockWithDropBlock(
            base_channels * 32,
            block_size=block_size, keep_prob=keep_prob, activation=activation
        )

        # Decoder
        self.d5 = DecoderBlockWithDropBlock(
            base_channels * 32, base_channels * 16,
            block_size=block_size, keep_prob=keep_prob, activation=activation
        )
        self.d4 = DecoderBlockWithDropBlock(
            base_channels * 16, base_channels * 8,
            block_size=block_size, keep_prob=keep_prob, activation=activation
        )
        self.d3 = DecoderBlockWithDropBlock(
            base_channels * 8, base_channels * 4,
            block_size=block_size, keep_prob=keep_prob, activation=activation
        )
        self.d2 = DecoderBlockWithDropBlock(
            base_channels * 4, base_channels * 2,
            block_size=block_size, keep_prob=keep_prob, activation=activation
        )
        self.d1 = DecoderBlockWithDropBlock(
...
ULiteDropBlock = ConfigurableULiteDropBlock